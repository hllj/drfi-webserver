...
class Region2Csv:
    ...
    @staticmethod
    def generate_similar_csv(rlist, comb_features, seg_path):
    ...
    @staticmethod
    def generate_seg_csv(rlist, features93, seg_path):
        in_segs = Region2Csv.get_in_segs(rlist, seg_path)
        """
        Each line of CSV will be like this:
    ...
    def combine_csv(path_list, all_csv_path):
        removed_path_list = path_list.copy()
        for path in path_list:
            if os.path.exists(path):
                try:
                    df = pd.read_csv(path)
                    removed_path_list.remove(path)
                except:
                    print("Error while reading csv {}".format(path))

        data = [
            pd.read_csv(path).values
            for path in path_list
            if path not in removed_path_list
        ]
